...
def gen_data():
    ...
    # UNI2TIPA_RTONE
    path_rtone = os.path.join(uni2tipa_dir, "uni2tipa-rtone.tsv")
    with open(path_rtone, encoding="utf-8") as f:
        reader = csv.reader(f, quoting=csv.QUOTE_NONE, delimiter="\t")
        data["UNI2TIPA_RTONE"] = {row[0]: row[1] for row in reader if len(row) >= 2}
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(f"UNI2TIPA_RTONE = {repr(data['UNI2TIPA_RTONE'])}\n\n")
